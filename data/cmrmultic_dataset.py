...
TR_CLASS_MAP_MMS_SRS= {'BG': 0,'LV_Bloodpool': 1, 'LV_Myocardium': 2,'RV_Bloodpool': 3,'abdomen': 7,'Body_fat': 5,'vessel': 9, 'extra_heart': 4, 'Lung': 8,'Skeletal': 6} # vendor C
...
# combine the dataset of center 1 and 2

class CmrmulticDataset(Pix2pixDataset):
    ...
    def modify_commandline_options(parser, is_train):
        ...
        parser.set_defaults(label_nc=10)
        ...
        parser.add_argument('--label_dir', type=str, required=False, default = '/data/sina/dataset/MMs_Processed/Vendor_C/Mask_Better_Cavity',
                            help='path to the directory that contains label images')
        parser.add_argument('--image_dir', type=str, required=False, default ='/data/sina/dataset/MMs_Processed/Vendor_C/Image' ,
                            help='path to the directory that contains photo images')
        # parser.add_argument('--label_dirb', type=str, required=False, default = '/data/sina/dataset/MMs_Processed/Center_2/Patch_Size_256/Mask',
        #                     help='path to the directory that contains label images')
        # parser.add_argument('--image_dirb', type=str, required=False, default ='/data/sina/dataset/MMs_Processed/Center_2/Patch_Size_256/Image' ,
        #                     help='path to the directory that contains photo images')
    ...
    def get_paths(self, opt):
        ...
        # assert os.path.exists(opt.label_dirb), 'list of masks  doesnt exist'
        # assert os.path.exists(opt.image_dirb), 'list of images doesnt exist'

        # img_listb = sorted(os.listdir(os.path.join(opt.image_dirb)))
        # msk_listb = sorted(os.listdir(os.path.join(opt.label_dirb)))
        ...
        # filename_pairsb = [(os.path.join(opt.image_dirb,x),os.path.join(opt.label_dirb, y)) for x,y in zip(img_listb,msk_listb)]
        # self.img_list = img_list + img_listb
        # self.msk_list = msk_list + msk_listb
        # self.filename_pairs = filename_pairs + filename_pairsb
    ...
    def initialize(self, opt):
        ...
        if opt.phase == 'train':
            train_transforms = Compose([
                ...
                # cmr_tran.Normalize(mean=0.5, std=0.5),
